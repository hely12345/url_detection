...
def get_whois(domain):
    ...
    for attempt in range(2):
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as ex:
            ...
            try:
                result = future.result(timeout=10)  # hard 10s wall-clock kill
                break
...
def fetch_whois_in_batches(domains, batch_size=200, max_workers=10):
    ...
    for i in range(total_batches):
        batch = [d for d in domains[i * batch_size:(i + 1) * batch_size]
                 if d not in whois_cache]  # skip already cached
...
def domain_reg_days(domain):
    ...
    if not entry or not entry[0] or not entry[1]:
        return None
    try:
        created = datetime.fromisoformat(entry[0].replace("+00:00", "").replace("Z", ""))
        expires = datetime.fromisoformat(entry[1].replace("+00:00", "").replace("Z", ""))
        return (expires - created).days
    except Exception:
        return None
    
df['DomainAgeDays']   = df['domain'].apply(domain_age_days)
df['DomainRegLength'] = df['domain'].apply(domain_reg_days)

df.drop(columns=['scheme', 'netloc', 'path', 'query', 'fragment'], inplace=True)

df.to_csv('Features_extracted_final.csv', index=False)



# print("whois_results length:", len(whois_results))
# print("whois_cache length:", len(whois_cache))

# # check a sample domain
# sample = df['domain'].dropna().iloc[0]
# print("Sample domain:", sample)
# print("In whois_results:", whois_results.get(sample))
# print("In whois_cache:", whois_cache.get(sample))