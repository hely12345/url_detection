...
@st.cache_data(ttl=86400, show_spinner=False)   # cache 24 h per domain
def safe_whois(domain: str, timeout: int = 10):
    ...
    # Fallback: RDAP over plain HTTPS (avoids port-43 blocks)
    try:
        r = requests.get(
...
def web_traffic_single(domain: str) -> int:
    try:
        ...
        if rank <= 10_000:      # ← matches training threshold
            return 1
        if rank <= 500_000:
            return 0
        return -1
...
def extract_features(inp: str) -> pd.DataFrame:
    ...
    # HTTPSDomainURL: does the literal string "https" appear inside domain/path?
